  ...
  for (pcb = pcbs; pcb < tailof(pcbs); pcb++) {
    if (pcb->state == UDP_PCB_STATE_OPEN) {
      if (pcb->local.port == key.port) {
        if (pcb->local.addr == key.addr || pcb->local.addr == IP_ADDR_ANY ||
...
static ssize_t udp_output(ip_endp_t src, ip_endp_t dst, const uint8_t *data,
  ...
  debugf("%s => %s, len=%zu", ip_endp_ntop(src, endp1, sizeof(endp1)),
         ip_endp_ntop(dst, endp2, sizeof(endp2)), total);
  udp_print(buf, total);
...
int udp_cmd_bind(int desc, ip_endp_t local) {
  ...
  if (exist) {
    errorf("already in use, desc=%d, want=%s, exist=%s", desc,
