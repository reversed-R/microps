...
static void tcp_pcb_release(struct tcp_pcb *pcb) {
  ...
  while (1) {
    ...
    debugf("release backlog entry, desc=%d, state=%s", tcp_pcb_desc(backlog),
           tcp_state_ntoa(backlog->state));
...
static void tcp_retrans_emit(void *arg, struct queue_entry *_entry) {
  ...
  if (timercmp(&now, &timeout, >)) {
    ...
    tcp_output_segment(entry->seq, pcb->rcv.nxt, entry->flg, pcb->rcv.wnd,
                       ...
                       pcb->remote);
    entry->last = now;
    entry->rto *= 2; /* expornential backoff */
  }
}
...
static void tcp_segment_arrives(struct seg_info *seg, uint8_t flags,
  ...
  case TCP_STATE_SYN_SENT:
    ...
    if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
      ...
      if (pcb->snd.una > pcb->iss) {
        ...
        /* NOTE: not specified in the RFC793,
         * but send window initialization required. */
        pcb->snd.wnd = seg->wnd;
        pcb->snd.wl1 = seg->seq;
        pcb->snd.wl2 = seg->ack;
        sched_task_wakeup(&pcb->task);
        /* ignore: continue processing at the sixth step below
         ...
         * queue the for processing after the ESTABLISHED state has been
         * reached. */
      }
    }
  ...
  case TCP_STATE_TIME_WAIT:
     ...
     * trimming off any portions that lie outside the window (including SYN and
     * FIN), and only processing further if the segment then begins at RCV.NXT.
     * Segments with heigher begining sequence numbers may be held for later
     * processing.
     * */
  }
  ...
  case TCP_STATE_CLOSING:
    ...
    case TCP_STATE_CLOSING:
      if (seg->ack == pcb->snd.nxt) {
        ...
        /* NOTE: set 2MSL timer, although it is not explicitly stated in the
         * RFC. */
...
static void tcp_input(const struct ip_hdr *iphdr, const uint8_t *data,
  ...
  debugf("%s => %s, le=%zu, dev=%s", ep1, ep2, len,
...
int tcp_init(void) {
  struct timeval interval = {0, 100000}; /* 100 ms */
...
int tcp_cmd_connect(int desc, ip_endp_t remote) {
  ...
  debugf("local=%s, remote=%s", ip_endp_ntop(local, ep1, sizeof(ep1)),
         ip_endp_ntop(local, ep2, sizeof(ep2)));
  if (local.addr == IP_ADDR_ANY) {
    iface = ip_route_get_iface(remote.addr);
    if (!iface) {
      errorf("iface not found that can reach remote address, addr=%s",
...
AGAIN:
  ...
  debugf("success, local=%s, remote=%s",
         ip_endp_ntop(pcb->local, ep1, sizeof(ep1)),
         ip_endp_ntop(pcb->local, ep2, sizeof(ep2)));
  lock_release(&lock);
  return 0;
}
...
int tcp_cmd_bind(int desc, ip_endp_t local) {
  ...
  if (exist) {
    errorf("already bound, exist=%s",
           ip_endp_ntop(exist->local, ep, sizeof(ep)));
    lock_release(&lock);
    return -1;
  }
  pcb->local = local;
...
int tcp_cmd_accept(int desc, ip_endp_t *remote) {
  ...
  debugf("success: desc=%d, local=%s, remote=%s", new_desc,
         ...
         ip_endp_ntop(new_pcb->local, ep2, sizeof(ep2)));
  lock_release(&lock);
  return new_desc;
}
